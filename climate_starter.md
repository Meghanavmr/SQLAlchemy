...
```
    ...
    {'_sa_instance_state': <sqlalchemy.orm.state.InstanceState at 0x174aba882e8>,
     'date': '2010-01-01',
     'id': 1,
     'prcp': 0.08,
     'station': 'USC00519397',
...
```
    ...
    {'_sa_instance_state': <sqlalchemy.orm.state.InstanceState at 0x174aba88c88>,
     'elevation': 3.0,
     'id': 1,
     'latitude': 21.2716,
     'longitude': -157.8168,
...
sel = [Measurement.date,Measurement.id, Measurement.prcp,Measurement.station, Measurement.tobs,
              Station.elevation,  Station.id,  Station.latitude,  Station.longitude,  Station.name
              ]
temp_sta = session.query(*sel).filter(Measurement.station == Station.station).all()
temp_sta
for record in temp_sta:
    (M_dt, M_id, M_prcp, M_tobs, S_ele, S_id, S_lat, S_lng, S_nm) = record
    print(record)
# sel_n = [temp_sta.station, 
#        func.max(temp_sta.M_tobs), 
#        func.avg(temp_sta.M_tobs)
#        ]
# active_station = session.query(*sel_n).\
#         (func.count(temp_sta.station).all().\
#     order_by(temp_sta.station).desc().all()
# active_station
...
```
    ...
    <ipython-input-23-e2c830d64c11> in <module>()
          9 temp_sta
         10 for record in temp_sta:
    ---> 11     (M_dt, M_id, M_prcp, M_tobs, S_ele, S_id, S_lat, S_lng, S_nm) = record
         12     print(record)
         13 # sel_n = [temp_sta.station,
...
```python
join_temp_sta = session.query(Measurement, Station).filter(Measurement.station == Station.station).limit(10).all()

for record in join_temp_sta:
